...
// ProductServer represents the HTTP server for the Core Utility application.
...
func NewProductServer(router *mux.Router, conf config.InterfaceConfig, log logger.InterfaceLogger) *productServer {
...
func (srv productServer) InitServer() {
	...
	srv.logger.Infof("Product running on %s", srv.conf.GetString("app.addr")+":"+srv.conf.GetString("app.port"))
